    ...
    <div className="h-full flex flex-col space-y-4 overflow-hidden">
      ...
      <div className="flex-1 overflow-hidden pb-2">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 h-full">
          {columns.map((col) => (
            <div key={col.status} className="flex flex-col h-full bg-slate-100 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto px-3 space-y-3 pb-4 custom-scrollbar">
                {requests.filter(r => r.status === col.status).map((req) => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mt-3">
                      <div className="text-[10px] bg-slate-50 dark:bg-slate-900 px-1.5 py-0.5 rounded border border-slate-100 dark:border-slate-800 truncate max-w-[100px]">
                        {req.division}
                      </div>
                ...
                {col.status === JobRequestStatus.Requests && canCreate && (
                  <button 
                    ...
                    className="w-full py-2 flex items-center gap-2 text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 text-xs font-medium rounded-lg hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors"
                  >
